...
EEG = pop_editeventfield( EEG, 'indices',  strcat('1:', int2str(length(EEG.event))), ...
    ...
    'nextValidTrial','NaN', 'num_all','NaN', 'num_block','NaN', 'num_inblock','NaN');
EEG = eeg_checkset( EEG );
...
for t = all_stimMarkers_eventNums %t = event numbers stored in all_stimMarkers_eventNums
    ...
    switch EEG.event(t).type     
        case ns_stimMarkers
            num_block_ns = floor(num_all_ns/40)+1;
            num_all_ns = num_all_ns+1;
            num_inblock_ns = num_all_ns - (num_block_ns-1)*40;
        case s_stimMarkers
            num_block_s = floor(num_all_s/40)+1;
            num_all_s = num_all_s+1;
            num_inblock_s = num_all_s - (num_block_s-1)*40;
    end
        ...
        if eventNum ~= 0
            ...
            switch EEG.event(t).type     
                case ns_stimMarkers
                    ...
                    EEG.event(eventNum).num_block = num_block_ns;
                    EEG.event(eventNum).num_all = num_all_ns;
                    EEG.event(eventNum).num_inblock = num_inblock_ns;
                case s_stimMarkers
                    EEG.event(eventNum).observation = 's';
                    EEG.event(eventNum).num_block = num_block_s;
                    EEG.event(eventNum).num_all = num_all_s;
                    EEG.event(eventNum).num_inblock = num_inblock_s;
            end
