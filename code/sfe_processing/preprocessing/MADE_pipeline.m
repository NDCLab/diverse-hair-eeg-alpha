...
addpath(pwd);
addpath('/home/data/NDClab/analyses/diverse-hair-eeg-alpha/code/sfe_processing/preprocessing');
...
fprintf('DEBUG main_dir: [%s]\n', main_dir);
fprintf('DEBUG rawdata_location_parent exists: %d\n', isdir(rawdata_location_parent));
...
parfor file_locater_counter = 1:length(subjects_to_process) %1:16
        ...
        rawdata_location = rawdata_location_parent;

        % Find files for this subject
        datafile_names = dir([rawdata_location filesep '*' char(subjects_to_process(file_locater_counter)) '*sfe*.vhdr']);
        ...
        if isempty(datafile_names)
                warning(['Cannot find sfe .vhdr file for subject ' char(subjects_to_process(file_locater_counter)) ' in ' rawdata_location_parent ', skipping.']);
            continue
        end
        ...
        output_location = char(fullfile('/home/data/NDClab/datasets/diverse-hair-eeg-dataset/derivatives/preprocessed/sfe_derivatives', subj, 'eeg'));
        output_report_path = char(fullfile('/home/data/NDClab/datasets/diverse-hair-eeg-dataset/derivatives/preprocessed/sfe_derivatives', 'MADE_preprocessing_report'));
	...
	channel_locations = loadbvef(channel_locations_file);	
        ...
        for subject=1:length(datafile_names)
            ...
            sess_match = regexp(fname_noext, '(s\d+r\d+e\d+|s\d+_r\d+_e\d+|s\d+-r\d+-e\d+)$', 'tokens');
            sess = sess_match{1}{1};
